...
class CFrameAnimation extends CFrame {
   ...
   public void Frame(Graphics g) {
      ...
      if (this.mDisplacement == null) {
         ...
         yinput = (int)(yscale * this.mInput[pcurrent]);
      }
      ...
      g.fillRect(500, 9 + yinput, 70, 20);
      g.setColor(Color.black);
      double coil = (200 + y - yinput - 9 - 20) / 10.0;
      g.drawLine(517, 29 + yinput, 529, (int)(29 + yinput + coil / 2.0));

      for (int p = 1; p < 10; p++) {
         if (p % 2 == 0) {
            g.drawLine(505, (int)(29 + yinput + (2 * p - 1) * coil / 2.0), 529, (int)(29 + yinput + (2 * p + 1) * coil / 2.0));
         } else {
            g.drawLine(529, (int)(29 + yinput + (2 * p - 1) * coil / 2.0), 505, (int)(29 + yinput + (2 * p + 1) * coil / 2.0));
         }
      }

      g.drawLine(505, (int)(29 + yinput + 19.0 * coil / 2.0), 517, 200 + y);
      ...
      g.setColor(Color.black);
      g.drawLine(545, 29 + yinput, 545, 29 + yinput + 113);
      g.drawLine(559, 29 + yinput, 559, 29 + yinput + 113);
      g.drawLine(545, 29 + yinput + 113, 559, 29 + yinput + 113);
      ...
      g.drawLine(
         ...
         this.mTraceThumbnail.y + this.mTraceClip.height / 8 / 2,
         (int)(this.mTraceThumbnail.x + (400.0 + 1.0 / dt) / 8.0),
         (int)(this.mTraceThumbnail.y + (this.mTraceClip.height / 2 + this.mXin * yscale) / 8.0)
      );
      g.drawLine(
         (int)(this.mTraceThumbnail.x + (400.0 + 1.0 / dt) / 8.0),
         (int)(this.mTraceThumbnail.y + (this.mTraceClip.height / 2 + this.mXin * yscale) / 8.0),
   ...
   public void Response() {
      ...
      for (int i = 0; i < 1599; i++) {
         this.mF[i] = -mW2 * (this.mX[i] - this.mInput[i])
            - 2.0 * mW * this.mXi * (this.mY[i] - (this.mInput[i + 1] - this.mInput[i]) / mdtx2);
         this.mX[i + 1] = this.mX[i] + this.mY[i] * mdtx2 / 2.0;
         this.mY[i + 1] = this.mY[i] + this.mF[i] * mdtx2 / 2.0;
         this.mF[i + 1] = -mW2 * (this.mX[i + 1] - this.mInput[i + 1])
            - 2.0 * mW * this.mXi * (this.mY[i + 1] - (this.mInput[i + 2] - this.mInput[i + 1]) / mdtx2);
         this.mX[i + 2] = this.mX[i] + this.mY[i + 1] * mdtx2 / 2.0;
         this.mY[i + 2] = this.mY[i] + this.mF[i + 1] * mdtx2 / 2.0;
         this.mF[i + 2] = -mW2 * (this.mX[i + 2] - this.mInput[i + 2])
            - 2.0 * mW * this.mXi * (this.mY[i + 2] - (this.mInput[i + 3] - this.mInput[i + 2]) / mdtx2);
         this.mX[i + 3] = this.mX[i] + this.mY[i + 2] * mdtx2;
         this.mY[i + 3] = this.mY[i] + this.mF[i + 2] * mdtx2;
         this.mF[i + 3] = -mW2 * (this.mX[i + 3] - this.mInput[i + 3])
            - 2.0 * mW * this.mXi * (this.mY[i + 3] - (this.mInput[i + 4] - this.mInput[i + 3]) / mdtx2);
         this.mX[i + 1] = this.mX[i] + (this.mY[i] + 2.0 * this.mY[i + 1] + 2.0 * this.mY[i + 2] + this.mY[i + 3]) * mdtx2 / 6.0;
         this.mY[i + 1] = this.mY[i] + (this.mF[i] + 2.0 * this.mF[i + 1] + 2.0 * this.mF[i + 2] + this.mF[i + 3]) * mdtx2 / 6.0;
   ...
   public void DrawInputGraph() {
      ...
      this.mTraceGC.drawLine(480 + (int)(1.0 / dt), 60, 480 + (int)(1.0 / dt), 60 + (int)(this.mXin * yscale));
      this.mTraceGC.drawLine(480 + (int)(1.0 / dt), 60 + (int)(this.mXin * yscale), 480 + (int)((1.0 + this.mPulsewidth) / dt), 60 + (int)(this.mXin * yscale));
      this.mTraceGC.drawLine(480 + (int)((1.0 + this.mPulsewidth) / dt), 60 + (int)(this.mXin * yscale), 480 + (int)((1.0 + this.mPulsewidth) / dt), 60);
